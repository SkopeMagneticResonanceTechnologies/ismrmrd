classdef AcquisitionHeader < handle
    ...
    methods
        ...
        function set(obj,range,hdr)
            % Sets a range of acquisition headers
            if length(range) ~= length(hdr.version)
                error('Selected range of does not match number of supplied headers.')
            end
            
            obj.version(1,range) = hdr.version(:);
            obj.flags(1,range) = hdr.flags(:);
            obj.measurement_uid(1,range) = hdr.measurement_uid(:);
            obj.scan_counter(1,range) = hdr.scan_counter(:);
            obj.acquisition_time_stamp(1,range) = hdr.acquisition_time_stamp(:);
            obj.physiology_time_stamp(:,range) = hdr.physiology_time_stamp(:);
            obj.number_of_samples(1,range) = hdr.number_of_samples(:);
            obj.available_channels(1,range) = hdr.available_channels(:);
            obj.active_channels(1,range) = hdr.active_channels(:);
            obj.channel_mask(:,range) = hdr.channel_mask(:);
            obj.discard_pre(1,range) = hdr.discard_pre(:);
            obj.discard_post(1,range) = hdr.discard_post(:);
            obj.center_sample(1,range) = hdr.center_sample(:);
            obj.encoding_space_ref(1,range) = hdr.encoding_space_ref(:);
            obj.trajectory_dimensions(1,range) = hdr.trajectory_dimensions(:);
            obj.sample_time_us(1,range) = hdr.sample_time_us(:);
            obj.position(:,range) = hdr.position(:);
            obj.read_dir(:,range) = hdr.read_dir(:);
            obj.phase_dir(:,range) = hdr.phase_dir(:);
            obj.slice_dir(:,range) = hdr.slice_dir(:);
            obj.patient_table_position(:,range) = hdr.patient_table_position(:);
            obj.idx.kspace_encode_step_1(1,range) = hdr.idx.kspace_encode_step_1(:);
            obj.idx.kspace_encode_step_2(1,range) = hdr.idx.kspace_encode_step_2(:);
            obj.idx.average(1,range) = hdr.idx.average(:);
            obj.idx.slice(1,range) = hdr.idx.slice(:);
            obj.idx.contrast(1,range) = hdr.idx.contrast(:);
            obj.idx.phase(1,range) = hdr.idx.phase(:);
            obj.idx.repetition(1,range) = hdr.idx.repetition(:);
            obj.idx.set(1,range) = hdr.idx.set(:);
            obj.idx.segment(1,range) = hdr.idx.segment(:);
            obj.idx.user(:,range) = hdr.idx.user(:);
            obj.user_int(:,range) = hdr.user_int(:);
            obj.user_float(:,range) = hdr.user_float(:);
        end
